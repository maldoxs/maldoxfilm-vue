    ...
    try {
      ...
      if (releaseName) {
        try {
          ...
          const r1 = await fetchImpl(q1, { headers }).then((r) => r.json());
      ...
      if (!data.data?.length) {
        try {
          ...
          const r2 = await fetchImpl(q2, { headers }).then((r) => r.json());
      ...
      const dlData = await fetchImpl(`/.netlify/functions/subhash?file_id=${encodeURIComponent(fileId)}`).then((r) =>
        r.json()
      );
      ...
      for (let attempt = 0; attempt < Math.min(ranked.length, MAX_DOWNLOAD_ATTEMPTS); attempt++) {
        ...
        const dl = await fetchImpl(`/.netlify/functions/subhash?file_id=${encodeURIComponent(candidateId)}`).then(
          (r) => r.json()
        );
        if (!dl.link) continue;
        const raw = await fetchImpl(dl.link).then((r) => r.text());
